const coures_model= require('./models/course');
const module_model= require('./models/modules');
const lesson_model= require('./models/lesson');
const courseproblem_model= require('./models/courseProblems');
const assessment_model= require('./models/Assessments');



async function main(){
    let is_present=await courseproblem_model.find();
    ...
    let problems_data=await courseproblem_model.insertMany(
        [
            {
    ...
    let lessons_data=await lesson_model.insertMany(
        [
    ...
    let modules_data=await module_model.insertMany(
        [
            {
    ...
    let courses_data=await coures_model.insertMany(
        [
            {
              courseid: 'CS101',
